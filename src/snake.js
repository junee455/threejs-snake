...
function generateFieldGeometry(width, height, cellSize) {
    ...
    // basicSquare.translate(0.5, 0.5, 0.5);
    // squareGeometry = squareGeometry.merge(basicSquare);
    // fieldGeometry.translate(squareSize + squareSize * 0.05, 0, 0);
    // fieldGeometry = mergeBufferGeometries([fieldGeometry, basicSquare]);
...
function setupRender(selector) {
    ...
    export function main(gameSettings) {
        ...
        const snakeSegments = new THREE.InstancedMesh(geometry, material, gameSettings.width * gameSettings.height);
        // snakeRenderer.scene.add(cube);
        
        
        let fieldGeometry = generateFieldGeometry(gameSettings.width, gameSettings.height, cellSize)
        ...
        // dummy.position.set(0, 0, cellSize / 2);
        // dummy.updateMatrix();
        // 
        // snakeSegments.setMatrixAt(0, dummy.matrix);
        // 
        // dummy.translateX(cellSize);
        // dummy.updateMatrix();
        // 
        // snakeSegments.setMatrixAt(1, dummy.matrix);
        ...
        */
        ...
        let tailSegment = [Math.floor(gameSettings.height / 2), Math.floor(gameSettings.width / 2) - 1];
        let headSegment = [Math.floor(gameSettings.height / 2), Math.floor(gameSettings.width / 2) + 1];
        
        this.field[Math.floor(gameSettings.height / 2)][Math.floor(gameSettings.width / 2) - 1] = Direction.right;
        this.field[Math.floor(gameSettings.height / 2)][Math.floor(gameSettings.width / 2)] = Direction.right;
        this.field[Math.floor(gameSettings.height / 2)][Math.floor(gameSettings.width / 2) + 1] = Direction.right;
        ...
        this.nextSegment = (segment, teleportEnabled, removePortal = false) => {
            ...
            if(teleportEnabled) {
                ...
                if((result[1] < 0) || (result[1] >= gameSettings.width)) {
                    ...
                    if(removePortal) {
                        if (portalIndex > -1)
                            this.portals.horizontal.splice(portalIndex, 1);
                    } else {
                        if (portalIndex === -1)
                            this.portals.horizontal.push(result[0]);
                    }
                }
                if((result[0] < 0) || (result[0] >= gameSettings.height)) {
                    ...
                    if(removePortal) {
                        if (portalIndex > -1)
                        this.portals.vertical.splice(portalIndex, 1);
                    } else {
                        if (portalIndex === -1)
                        this.portals.vertical.push(result[1]);
                    }
                }
                ...
                // snake hits the wall
                if(result[0] >= gameSettings.height
                    || result[0] < 0
            ...
            function getSegmentCoords(segment) {
                return [segment[1] * cellSize - (gameSettings.width / 2) * cellSize + cellSize * 0.5,
                segment[0] * cellSize - (gameSettings.height / 2) * cellSize + cellSize * 0.5];
            }
            ...
            this.renderPortals = () => {
                ...
                snakeSegments.count += this.portals.horizontal.length * 2 + this.portals.vertical.length;
                dummy.scale.set(0.01, 1.5, 4);
                dummy.updateMatrix();
                // rendering horizontal portals
                for(let i = 0; i < this.portals.horizontal.length; i++) {
                    const yPosition = (this.portals.horizontal[i] - gameSettings.height / 2 + 0.5) * cellSize;
                    dummy.position.set(-gameSettings.width / 2 * cellSize,
                        yPosition,
                        0);
                    dummy.updateMatrix();
                ...
                for(let i = 0; i < this.portals.vertical.length; i++) {
                    const xPosition = (this.portals.vertical[i] - gameSettings.width / 2 + 0.5) * cellSize;
                    dummy.position.set(xPosition, gameSettings.height / 2 * cellSize, 0);
                    dummy.updateMatrix();
                    snakeSegments.setMatrixAt(firstInstance + this.portals.horizontal.length * 2 + i, dummy.matrix);
            ...
            this.renderSnake = () => {
                ...
                this.animateHead(currentTime);
                this.animateTail(currentTime);
            ...
            this.dropApple = () => {
                ...
                const newApple = [getRandomInt(gameSettings.height), getRandomInt(gameSettings.width)];
            ...
            this.animateTail = (time) => {
                ...
                const timeDelta = (time - movesPast * this.movementSpeed * 1000) / (this.movementSpeed * 1000);
            ...
            this.animateHead = (time) => {
                const segment = preHeadSegment;
                let delta = [0, 0];
                switch (this.getCellValue(segment)) {
                    case Direction.right:
                    delta[0] = cellSize;
            ...
            this.render = (time) => {
                ...
                if(time - movesPast * this.movementSpeed * 1000 > this.movementSpeed * 1000) {
                    movesPast++;
                    if(!this.isGameOver) {
                        this.nextTurn();
                    } else {
